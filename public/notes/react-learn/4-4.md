...
注意，在配置列的时候，有一些列选项是属于 *procomponents* 新增的，所以有些属性我们需要参阅 *procomponents* 的文档
...
如果是单纯的渲染某一个值，那么直接配置 *dataIndex* 即可，但是很多时候，我们是根据数据对应的值渲染成其他的东西
...
被设置了 *name* 属性的 *ormtem* 包装的控件，表单控件会自动添加 *value* (或 *valuePropName* 指定的其他属性)*onChange*(*trigger* 指定的其他属性)，数据同步将被 *Form* 接管，这会导致问题:
